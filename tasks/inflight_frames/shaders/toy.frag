...
mat3 rotateX(float al, out mat3 reverse) 
{
    float c = cos(al);
    float s = sin(al);
    reverse  =  mat3(
...
mat3 rotateZ(float al, out mat3 reverse) 
{
    float c = cos(al);
    float s = sin(al);
    reverse  =  mat3(
 ...
 out vec3 offset, out float mixC) 
{
    ...
    {
        time -= DELAY_TIME;
        offset = vec3(2.0 * cos(time), 1.0 + 1.5 * sin(time), 0.25 * sin(time));
    } else 
    {
...
vec3 triplanar(vec3 p, vec3 offset, vec3 n, in mat3 m) 
{
    p -= offset;
...
void needlessJob(float time) 
{
    for (float c = 0.0; c < 100000.0; c = c + 0.01) {
        for (float s = 0.0; s < 100000.0; s = s + 0.01) {
            mat3 m = mat3(
        vec3( c,c+s,s),
        vec3( time,1,s-c),
        vec3(-s,15,c)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord) 
{
    ...
    mat3 reverse, reverseX, reverseY;
    mat3 m = rotateY(6.0 * mouse.x, reverseX) * rotateX(2.0 * mouse.y, reverseY);
    reverse = reverseY * reverseX;
    ...
    if (hit) 
    {
        ...
        vec3 boxColor   = vec3(triplanar(p * reverse, offset, n, m));
